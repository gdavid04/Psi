/*
 * This class is distributed as part of the Psi Mod. Get the Source Code in
 * github: https://github.com/Vazkii/Psi
 *
...
/**
 * Holder class for a spell's piece grid. Pretty much all internal, nothing to
 * see here.
 */
public final class SpellGrid {
	...
	public boolean shift(SpellParam.Side side, boolean doit) {
		...
		if (exists(leftmost + side.offx, topmost + side.offy)
				&& exists(rightmost + side.offx, bottommost + side.offy)) {
			if (!doit) {
				return true;
			}
	...
	private SpellPiece getPieceAtSide(Multimap<SpellPiece, SpellParam.Side> traversed, int x, int y,
			SpellParam.Side side) throws SpellCompilationException {
	...
	public SpellPiece getPieceAtSideWithRedirections(List<SpellPiece> unused, int x, int y, SpellParam.Side side)
	...
	/**
	 * @param walker a callback that incrementally gets called on each redirector
	 *               reached
	 */
